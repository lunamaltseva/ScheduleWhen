...
const COMP_LABS: { depts: string[]; rooms: string[]; note: string }[] = [
  { depts: ['SFW', 'AMI'], rooms: ['G30', 'G31', '432', '433'], note: 'Computer Science' },
  { depts: ['JMC', 'TCMA'], rooms: ['C07'], note: 'Media Production' },
...
export function getRoomGroups(target: number, filterDepts: string[]): RoomGroup[] {
  ...
  if (target >= 60) {
    ...
    groups.push({ label: 'Medium Rooms', note: '30–60 seats', rooms: ['G34', '220', '330', '435'] });
    groups.push({ label: 'Large Rooms', note: '60+ seats', rooms: ['410', '434', '440', 'Forum'] });
  } else if (target >= 10) {
